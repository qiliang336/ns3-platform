...
class Ns3AIRL:
    ...
    def Acquire(self):
        while not self.isFinish() and self.GetVersion() % self.mod != self.res:
            print("self.isFinish=",self.isFinish(),",self.GetVersion=",self.GetVersion(),",self.mod =",self.mod,",self.res=",self.res)
            # mod=2,res=1
            pass
        if self.isFinish():
            self.finished = True
...
class Experiment:
    ...
    def __init__(self, shmKey, memSize, programName, path, using_waf = True):
        if self._created:
