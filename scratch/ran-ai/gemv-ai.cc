...
{
  // LogComponentEnable("GemvIntegrationExample",LOG_LEVEL_ALL);
  // LogComponentEnable("ShmPool",LOG_LEVEL_ALL);
  // LogComponentEnable("MmWaveNetDevice",LOG_LEVEL_ALL);
  uint32_t numUes = 1;
  uint32_t firstVehicleIndex = 0; //firstVehicleIndex-来自python
  ...
  //ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(0, 100, 0));
    ...
    /* 参照examples/matrix-topology/matrix-topology.cc
