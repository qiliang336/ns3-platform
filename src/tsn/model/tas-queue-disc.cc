...
{
  //NS_LOG_LOGIC("mac address: " << item->GetAddress());
  ...
  if(item->GetSize() > m_Mtu)
    {
      ...
      NS_LOG_DEBUG ("DropBeforeEnqueue: PAKET_SIZE_EXCEEDED_DROP " << m_Mtu << " < " << item->GetSize());
      return false;
    }
  ...
  if(m_trustQostag || item->GetPacket ()->PeekPacketTag (qosTag))
    {
      ...
      NS_LOG_DEBUG ("Tag found: " << qosTag.GetPriority () << " Use the previous priority.");
    }
  else
    {
      ...
      if (childclass == PacketFilter::PF_NO_MATCH)
          ...
          {
            NS_LOG_DEBUG ("Packet filters returned " << childclass << " Use the specified priority.");
  ...
  NS_LOG_DEBUG ("Packet: " << item->GetPacket() << " Size: " << item->GetSize());
...
TasQueueDisc::SetTransmitting(Ptr<const QueueDiscItem> item)
{
  ...
  NS_LOG_LOGIC("Transmitting until " << Simulator::Now() + sleepTime << " Packet: " << item->GetPacket());
