...
public:
  ...
  /**
   * Creates a new session with the specified partner, type and local clock of AP. The basic initialization of the FtmSession is
   * also done here.
   *
   * \param partner the partner Mac48Address
   * \param type the session type
   * \param clk_AP local clock of AP
   * \return the newly created FtmSession
   */
  Ptr<FtmSession> CreateNewSession (Mac48Address partner, FtmSession::SessionType type, Ptr<LocalClock> clk_AP);
