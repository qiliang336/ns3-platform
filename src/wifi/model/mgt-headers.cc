...
NS_OBJECT_ENSURE_REGISTERED (MgtTWTReqHeader);

MgtTWTReqHeader::MgtTWTReqHeader ()
  : m_reasonCode (1)
{
}

TypeId
MgtTWTReqHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MgtTWTReqHeader")
    .SetParent<Header> ()
    .SetGroupName ("Wifi")
    .AddConstructor<MgtTWTReqHeader> ()
  ;
  return tid;
}

TypeId
MgtTWTReqHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void
MgtTWTReqHeader::Print (std::ostream &os) const
{
}

uint32_t
MgtTWTReqHeader::GetSerializedSize (void) const
{
  uint32_t size = 0;
  size += 2; //DelBa parameter set
  size += 2; //Reason code
  size += 2; //Station's ID
  size += 2; //wake time
  size += 2; //wake duration
  size += 2; //wake interval
  return size;
}

void
MgtTWTReqHeader::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
  i.WriteHtolsbU16 (GetParameterSet ());
  //i.WriteHtolsbU16 (m_reasonCode);
  i.WriteHtolsbU16 (m_sta_id);
  i.WriteHtolsbU16 (m_sp_id);
  i.WriteHtolsbU16 (m_waketime);
  i.WriteHtolsbU16 (m_wakeduration);
  i.WriteHtolsbU16 (m_wakeinterval);
}

uint32_t
MgtTWTReqHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  SetParameterSet (i.ReadLsbtohU16 ());
  m_reasonCode = i.ReadLsbtohU16 ();
  m_sta_id = i.ReadLsbtohU16();
  m_sp_id = i.ReadLsbtohU16();
  m_waketime = i.ReadLsbtohU16 ();
  m_wakeduration = i.ReadLsbtohU16 ();
  m_wakeinterval = i.ReadLsbtohU16 ();
  return i.GetDistanceFrom (start);
}

bool
MgtTWTReqHeader::IsByOriginator (void) const
{
  return (m_initiator == 1) ? true : false;
}

uint8_t
MgtTWTReqHeader::GetTid (void) const
{
  NS_ASSERT (m_tid < 16);
  uint8_t tid = static_cast<uint8_t> (m_tid);
  return tid;
...
void
MgtTWTReqHeader::SetByOriginator (void)
{
  m_initiator = 1;
}

void
MgtTWTReqHeader::SetByRecipient (void)
{
  m_initiator = 0;
}

void
MgtTWTReqHeader::SetTid (uint8_t tid)
{
  NS_ASSERT (tid < 16);
  m_tid = static_cast<uint16_t> (tid);
}
...
uint16_t
MgtTWTReqHeader::GetParameterSet (void) const
{
  uint16_t res = 0;
  res |= m_initiator << 11;
  res |= m_tid << 12;
...
void
MgtTWTReqHeader::SetParameterSet (uint16_t params)
{
  m_initiator = (params >> 11) & 0x01;
  m_tid = (params >> 12) & 0x0f;
}
...
void
MgtTWTReqHeader::SetStaID (uint16_t id)
{
  m_sta_id = id;
}

uint16_t
MgtTWTReqHeader::GetStaID (void) const
{
  return m_sta_id;
}

void
MgtTWTReqHeader::SetSpID (uint16_t id)
{
  m_sp_id = id;
}

uint16_t
MgtTWTReqHeader::GetSpID (void) const
{
  return m_sp_id;
}

void
MgtTWTReqHeader::SetWakeTime (uint16_t time)
{
  m_waketime = time;
}

uint16_t
MgtTWTReqHeader::GetWakeTime (void) const
{
  return m_waketime;
}

void
MgtTWTReqHeader::SetWakeDuration (uint16_t time)
{
  m_wakeduration = time;
}

uint16_t
MgtTWTReqHeader::GetWakeDuration (void) const
{
  return m_wakeduration;
}

void
MgtTWTReqHeader::SetWakeInterval (uint16_t time)
{
  m_wakeinterval = time;
}

uint16_t
MgtTWTReqHeader::GetWakeInterval (void) const
{
