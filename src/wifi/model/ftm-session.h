...
public:
  ...
  /**
   * Set the callback when time offsets between AP and STA needs to calaculated. As of now, the offset is calculated
   * during second FTM of the first burst.
   *
   * \param callback the user function callback
   */
  void SetOffsetCorrectionCallback (Callback<void, Ptr<FtmSession::FtmDialog>, FtmSession> callback);
  
  /**
   * Set the callback when the propagation delay needs to be calculated. As of now, the Propgation delay is calculated 
   * during last FTM of last the burst
   *
   * \param callback the user function callback
   */
  void SetPropgationDelayCallback (Callback<void, FtmSession> callback);
  /**
  ...
  /**
   * Set local clock of AP for time synchronization.
   *
   * \param clockAP local clock of AP
   */
  void SetAPTSClock (Ptr<LocalClock> clockAP);

   /**
   * Set local clock of STA for time synchronization.
   *
   * \param clockSTA local clock of STA
   */
  void SetSTATSClock (Ptr<LocalClock> clockSTA);
  
   /**
   * Get the local clock of AP used for time synchronization.
   *
   * \return local clock of AP
   */
  Ptr<LocalClock> GetAPTSClock (void);

  /**
   * Get the local clock of STA used for time synchronization.
   *
   * \return local clock of STA
   */
  Ptr<LocalClock> GetSTATSClock (void);
...
private:
  ...
  Ptr<LocalClock> clock_AP; /* Local clock of AP to be associated to the FTM session */
  Ptr<LocalClock> clock_STA; /* Local clock of STA to be associated to the FTM session */
  ...
  Callback<void, Ptr<FtmSession::FtmDialog>, FtmSession> offset_correction_callback;  //!< Offset Correction callback
  Callback<void, FtmSession> propagation_delay_callback; //!< Propagation delay correction callback
  /**
  ...
  /**
   * Calculates Offset between the AP and STA clocks.
   *
   * \param dialog the FtmDialog to calculate offset
   */
  void OffsetCorrection (Ptr<FtmDialog> dialog);
