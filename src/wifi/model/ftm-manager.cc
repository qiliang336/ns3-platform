...
FtmManager::PhyTxBegin(Ptr<const Packet> packet, double num)
{
  ...
  if(hdr.IsMgt() && hdr.IsAction()) {
      ...
      if(action_hdr.GetCategory() == WifiActionHeader::PUBLIC_ACTION) {
		  ...
		  /* Set T1 from the local clock of AP */
		  session->SetT1(ftm_resp_hdr.GetDialogToken(), (session->GetAPTSClock()->GetLocalTime().GetPicoSeconds() & 0x0000FFFFFFFFFFFF));
		  received_packets = 0;
                  awaiting_ack = true;
      ...
      if(sending_ack && sent_packets == 1) {
          if(m_ack_to == hdr.GetAddr1()) {
              ...
              if (session != 0)
                {
		  /* Set T3 from the local clock of STA */
		  session->SetT3(m_current_rx_packet.ftm_res_hdr.GetDialogToken(), ( session->GetSTATSClock()->GetLocalTime().GetPicoSeconds() & 0x0000FFFFFFFFFFFF));
                }
          }
      }
  }
  ...
  if(hdr.GetAddr1() == m_mac_address){
          ...
          if(action_hdr.GetCategory() == WifiActionHeader::PUBLIC_ACTION) {
              ...
              if(action_hdr.GetAction().publicAction == WifiActionHeader::FTM_RESPONSE) {
                  ...
                  if (session != 0 && ftm_res_hdr.GetDialogToken() != 0)
                    {
		      /* Set T2 from the local clock of STA */
		      session->SetT2(ftm_res_hdr.GetDialogToken(), ( session->GetSTATSClock()->GetLocalTime().GetPicoSeconds() & 0x0000FFFFFFFFFFFF));
		      PacketInPieces pieces;
                      pieces.mac_hdr = hdr;
                      pieces.action_hdr = action_hdr;
      ...
      if(hdr.IsAck()) {
          if(awaiting_ack && received_packets == 1) {
              ...
              if (session != 0)
                {
		/* Set T4 from the local clock of AP */
		session->SetT4(m_current_tx_packet.ftm_res_hdr.GetDialogToken(), ( session->GetAPTSClock()->GetLocalTime().GetPicoSeconds() & 0x0000FFFFFFFFFFFF ));
                }
          }
...
Ptr<FtmSession>
FtmManager::CreateNewSession (Mac48Address partner, FtmSession::SessionType type, Ptr<LocalClock> clk_AP)
{
  if(FindSession(partner) == 0 && !CheckSessionBlocked (partner) && partner != m_mac_address)
    {
      ...
      new_session->SetAPTSClock(clk_AP);
...
FtmManager::ReceivedFtmRequest (Mac48Address partner, FtmRequestHeader ftm_req)
{
  ...
  if (session == 0)
  {
      /* This part of code involves finding the FTM responder node and associating 
       * the local clock of AP to the FTM responder session. In other words when 
       * the FTM Responder (AP) initiates the session, the AP's local clock has 
       * to be associated to this particular session so that we are able to fetch the
       * timestamps of the local clock of AP.*/
      for (uint32_t i = 0; i < NodeList::GetNNodes (); i++)
      {
          ...
          {
              Ptr<NetDevice> pThisDev = node->GetDevice (j);
	      if(pThisDev->GetObject<WifiNetDevice> () != NULL)
	      {
		  Ptr<WifiNetDevice>dev =DynamicCast<WifiNetDevice> (pThisDev);
		  Ptr<WifiMac> mac = dev->GetMac ();

		  if ( m_mac_address == mac->GetAddress())
		  {
			check = 1;
		  	break;
		  }
      ...
      Ptr<LocalClock> clk_AP = node->GetObject<LocalClock>();
      session = CreateNewSession(partner, FtmSession::FTM_RESPONDER, clk_AP);
