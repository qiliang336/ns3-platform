...
#include "ns3/log.h"
#include "st-multi-user-scheduler.h"
...
StMultiUserScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::StMultiUserScheduler")
    ...
    .AddAttribute ("UseCentral26TonesRus",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&StMultiUserScheduler::m_useCentral26TonesRus),
                   MakeBooleanChecker ())
    .AddAttribute ("MaxCredits",
                   ...
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&StMultiUserScheduler::m_maxCredits),
                   MakeTimeChecker ())
  ;
  return tid;
}

StMultiUserScheduler::StMultiUserScheduler ()
...
void
StMultiUserScheduler::DoInitialize (void)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_apMac != nullptr);
  m_apMac->TraceConnectWithoutContext ("AssociatedSta",
                                       MakeCallback (&StMultiUserScheduler::NotifyStationAssociated, this));
  m_apMac->TraceConnectWithoutContext ("DeAssociatedSta",
                                       MakeCallback (&StMultiUserScheduler::NotifyStationDeassociated, this));
...
void
StMultiUserScheduler::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_staList.clear ();
  m_candidates.clear ();
...
StMultiUserScheduler::SelectTxFormat (void)
{
  NS_LOG_FUNCTION (this);
...
MultiUserScheduler::TxFormat
StMultiUserScheduler::TrySendingBsrpTf (void)
{
  NS_LOG_FUNCTION (this);
...
MultiUserScheduler::TxFormat
StMultiUserScheduler::TrySendingBasicTf (void)
{
  NS_LOG_FUNCTION (this);
  ...
  for (const auto& candidate : m_candidates)
    {
      ...
      if (queueSize > 0)
        {
          NS_LOG_DEBUG ("Add STA: " << candidate.first->address << " to ulCandidates");
...
void
StMultiUserScheduler::NotifyStationAssociated (uint16_t aid, Mac48Address address)
{
...
void
StMultiUserScheduler::NotifyStationDeassociated (uint16_t aid, Mac48Address address)
{
  NS_LOG_FUNCTION (this << aid << address);
...
MultiUserScheduler::TxFormat
StMultiUserScheduler::TrySendingDlMuPpdu (void)
{
  NS_LOG_FUNCTION (this);
...
StMultiUserScheduler::ComputeDlMuInfo (void)
{
  NS_LOG_FUNCTION (this);
...
void
StMultiUserScheduler::AssignRuIndices (WifiTxVector& txVector)
{
...
StMultiUserScheduler::ComputeUlMuInfo (void)
{
  NS_LOG_FUNCTION (this);
