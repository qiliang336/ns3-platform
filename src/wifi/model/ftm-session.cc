...
FtmSession::FtmSession ()
{
  ...
  /* Create Objects for local clocks of AP and STA for theirs respective sessions */
  clock_AP = CreateObject<LocalClock> ();
  clock_STA = CreateObject<LocalClock> ();
  ...
  offset_correction_callback = MakeNullCallback<void, Ptr<FtmSession::FtmDialog>, FtmSession> (); /* Offset Correction Callback */
  propagation_delay_callback = MakeNullCallback<void, FtmSession> (); /* Propagation delay manager callback */
...
FtmSession::~FtmSession ()
{
  ...
  session_over_ftm_manager_callback = MakeNullCallback<void, Mac48Address> ();
  session_over_callback = MakeNullCallback<void, FtmSession> ();
  offset_correction_callback = MakeNullCallback<void, Ptr<FtmSession::FtmDialog>, FtmSession> ();
  propagation_delay_callback = MakeNullCallback<void, FtmSession> ();
  block_session = MakeNullCallback<void, Mac48Address, Time> ();
  live_rtt = MakeNullCallback<void, int64_t> ();
  session_override = MakeNullCallback<void, Mac48Address, FtmRequestHeader> ();
}
...
Ptr<LocalClock>
FtmSession::GetAPTSClock (void)
{
  return clock_AP;
}

Ptr<LocalClock>
FtmSession::GetSTATSClock (void)
{
  return clock_STA;
}
  ...
  if (ftm_res.GetFollowUpDialogToken() != 0)
    {
	  ...
	  if(m_number_of_bursts_remaining == std::pow(2,m_ftm_params.GetNumberOfBurstsExponent()) - 1)
	  {
	  	OffsetCorrection(follow_up_dialog);
	  }
...
void
FtmSession::SetOffsetCorrectionCallback (Callback<void, Ptr<FtmSession::FtmDialog>, FtmSession> callback)
{
  offset_correction_callback = callback;
}

void
FtmSession::SetPropgationDelayCallback (Callback<void, FtmSession> callback)
{
  propagation_delay_callback = callback;
}
...
void
FtmSession::OffsetCorrection (Ptr<FtmSession::FtmDialog> dg)
{
	offset_correction_callback (dg, *this);
}
...
//  if (m_session_over_callback_set && m_session_type == FTM_INITIATOR) //to fix break from session_override
  if (m_session_over_callback_set)
    {
      propagation_delay_callback(*this);
...
void
FtmSession::SetAPTSClock (Ptr<LocalClock> clockAP)
{
  clock_AP = clockAP;
}
void
FtmSession::SetSTATSClock (Ptr<LocalClock> clockSTA)
{
  clock_STA = clockSTA;
}
