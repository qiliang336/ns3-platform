...
public:
  ...
  Mac48Address GetBssid (uint8_t linkId) const override;
  void Enqueue (Ptr<Packet> packet, Mac48Address to, Mac48Address from) override;
  bool SupportsSendFrom (void) const override;
  void SetWifiPhy (const Ptr<WifiPhy> phy) override;
  Ptr<WifiPhy> GetWifiPhy (uint8_t linkId) const override;
  ...
  typedef Callback<void, Ptr<WifiMacQueueItem>> VsaCallback;
  void SetVSACallback (VsaCallback vsaCallback);
  Ptr<WifiRemoteStationManager> GetWifiRemoteStationManager (uint8_t linkId) const override;
