    ...
    static TypeId tid =
        TypeId("ns3::PacketSocketClient")
            ...
            .AddAttribute(
                ...
                //UintegerValue(100),   //in 3.38
                UintegerValue(1),   // in 3.34
                MakeUintegerAccessor(&PacketSocketClient::m_maxPackets),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute("Interval",
                          "The time to wait between packets",
                          //TimeValue(Seconds(1.0)), //in 3.38
                          TimeValue(Seconds(10)),   //in 3.34
                          MakeTimeAccessor(&PacketSocketClient::m_interval),
                          MakeTimeChecker())
            .AddAttribute("PacketSize",
                          "Size of packets generated (bytes).",
                          //UintegerValue(1024),    //in 3.38
                          UintegerValue(12),        //in 3.34
...
PacketSocketClient::Send()
{
    ...
    if ((m_socket->Send(p)) >= 0)
    {
        ...
        //NS_LOG_INFO("TraceDelay TX " << m_size << " bytes to "    //in 3.38
        NS_LOG_UNCOND ("TraceDelay TX " << p->GetSize() << " bytes to "     //in3.34
...
//in 3.34
void
PacketSocketClient::SetPacket (Ptr <Packet> ps)
{
NS_LOG_FUNCTION (this);
m_ps = ps;
